class Nem12ComparisonUI {
    constructor() {
        this.beforeFiles = { files: [], names: [] };
        this.afterFiles = { files: [], names: [] };
        this.beforeFileInfos = [];
        this.afterFileInfos = [];
        this.filePairs = [];
        this.pairResults = [];
        this.activeTab = null;
        this.results = null;
        this.currentDetails = []; // Store current filtered/sorted details
    ...
    updateFileMappingDisplay() {
        ...
        const sortedBeforeFiles = [...this.beforeFileInfos].sort((a, b) => {
            ...
            if (nmiA !== nmiB)
                return nmiA.localeCompare(nmiB);
    ...
    autoMatchFiles() {
        ...
        for (const beforeInfo of this.beforeFileInfos) {
            ...
            const match = this.afterFileInfos.find(afterInfo => {
                if (usedAfterFiles.has(afterInfo.name))
                    return false;
    ...
    togglePairCollapse(pairId) {
        const contentEl = document.getElementById(`content_${pairId}`);
        const toggleBtn = document.querySelector(`[data-pair-id="${pairId}"]`);
            ...
            for (let i = 0; i < enabledPairs.length; i++) {
                ...
                try {
                    ...
                    const result = await Promise.race([
                        ...
                        new Promise((_, reject) => setTimeout(() => reject(new Error('Comparison timeout after 5 minutes')), 300000))
                    ]);
    ...
    async performPairComparison(pair) {
        ...
        let issues;
        try {
            issues = this.parseAndCompareNem12(beforeContent, afterContent, pair.before.name, pair.after.name);
        }
        catch (error) {
    ...
    parseAndCompareNem12(beforeContent, afterContent, beforeFileName, afterFileName) {
        const issues = [];
        try {
            // Parse CSV content
    ...
    displayMultipleResults() {
        ...
        if (!resultsSection || !pairResultsContainer)
            return;
        ...
        if (overallHighEl)
            overallHighEl.textContent = overallCritical.toString();
        if (overallMediumEl)
            overallMediumEl.textContent = overallMismatch.toString();
        if (overallTotalEl)
            overallTotalEl.textContent = overallTotal.toString();
    ...
    displayResults(results) {
        ...
        if (highCountEl)
            highCountEl.textContent = criticalCount.toString();
        if (mediumCountEl)
            mediumCountEl.textContent = mismatchCount.toString();
        if (totalCountEl)
            totalCountEl.textContent = totalIssues.toString();
    ...
    filterTable(searchTerm) {
        ...
        if (term) {
            filtered = this.currentDetails.filter(detail => {
                return ((detail.sr?.toString() || '').toLowerCase().includes(term) ||
        ...
        const tableEl = activeTabContent ? activeTabContent.querySelector('.pair-results-table') : null;
        ...
        const sorted = [...this.currentDetails].sort((a, b) => {
            ...
            switch (columnIndex) {
                ...
                case 6:
                    ...
                    break; // field_name
                case 7:
                    aVal = a.after_cell_location || '';
                    bVal = b.after_cell_location || '';
                    break; // after_cell_location
                case 8:
                    aVal = a.before_value || '';
                    bVal = b.before_value || '';
                    break; // before_value
    ...
    renderTable(details, metadata, container) {
        ...
        const searchInputInline = document.getElementById('searchInputInline');
        if (searchInputInline) {
            searchInputInline.removeEventListener('input', this.searchHandler);
        ...
        if (resetBtnInline) {
            resetBtnInline.removeEventListener('click', this.resetHandler);
            this.resetHandler = () => {
                this.resetView();
            };
    ...
    generateResultsTable(details, metadata) {
